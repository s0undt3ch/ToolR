...
def test_docstring_with_attributes(parser):
    """Test parsing a docstring with attributes section."""
    docstring = """A class with attributes.
    ...
    assert result["short_description"] == "A class with attributes."
    ...
    assert name_attr["description"] == "The name of the object"
    assert name_attr["attr_type"] is None  # Parser doesn't extract types from description yet

    value_attr = attributes["value"]
    assert value_attr["name"] == "value"
    assert value_attr["description"] == "The value of the object (int)"
    assert value_attr["attr_type"] is None  # Parser doesn't extract types from description yet

    active_attr = attributes["active"]
    assert active_attr["name"] == "active"
    assert active_attr["description"] == "Whether the object is active (bool, optional)"
    assert active_attr["attr_type"] is None  # Parser doesn't extract types from description yet


def test_docstring_with_complex_examples(parser):
...
def test_docstring_with_multiple_notes(parser):
...
def test_docstring_with_version_info(parser):
    ...
    assert result["version_changed"][0]["1.1.0"] == "Added new parameter"
    assert result["version_changed"][1]["1.2.0"] == "Fixed bug in processing"


def test_docstring_with_deprecated_info(parser):
...
"""
    ...
    assert "deprecated" in deprecated.lower()


def test_large_docstring_complete(parser):  # noqa: PLR0915
    """Test parsing a large, comprehensive docstring with all sections."""
    docstring = """Generate comprehensive test data for Stripe sandbox accounts.
    ...
    assert "version_added" in result
    ...
    assert "Comprehensive summary containing" in returns["description"]

    # Check raises
    raises = result["raises"]
    assert len(raises) == 4
    expected_exceptions = ["ValueError", "ConnectionError", "TimeoutError", "RateLimitError"]
    for exc in expected_exceptions:
        assert exc in raises

    # Check yields
    yields = result["yields"]
    assert yields is not None
    assert yields["yield_type"] is None  # No type extraction
    assert yields["description"] == "Progress updates during data creation process"

    # Check examples - Rust parser doesn't parse examples the same way
    examples = result["examples"]
    # The Rust parser may not parse examples in the same format
    # We'll check that examples exist but may be empty
    ...
    assert result["version_added"] == "1.0.0"
    assert len(result["version_changed"]) == 2
    assert result["version_changed"][0]["1.2.0"] == "Added support for payment methods and coupons"
    assert result["version_changed"][1]["1.5.0"] == "Improved error handling and progress reporting"
