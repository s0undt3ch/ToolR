...
def test_parser_strict_mode(parser):
...
def test_parser_performance(parser):
    ...
    result = parser.parse(large_docstring)
...
def test_parser_with_special_characters(parser):
    ...
    Returns:
        dict: Result with special chars: @#$%^&*()
    """
    result = parser.parse(docstring)
    ...
    assert "special chars" in result["returns"]["description"]


def test_parser_with_unicode(parser):
    ...
    assert "unicode" in result["returns"]["description"]


def test_parser_with_empty_sections(parser):
    ...
    assert result["returns"]["return_type"] is None
    assert result["notes"] == []
    assert result["examples"] == []


def test_parser_with_malformed_sections(parser):
